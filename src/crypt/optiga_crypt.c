...
_STATIC_H void optiga_crypt_generic_event_handler(void *p_ctx, optiga_lib_status_t event) {
...
_STATIC_H optiga_lib_status_t optiga_crypt_generate_keypair(
    optiga_crypt_t *me,
    uint8_t cmd_param,
    uint8_t key_usage,
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == p_public_key)
            || (NULL == p_private_key) || (NULL == p_public_key_length)) {
        ...
        return_value = optiga_cmd_gen_keypair(
            me->my_cmd,
            (uint8_t)cmd_param,
            (optiga_gen_keypair_params_t *)p_params
        );
...
_STATIC_H optiga_lib_status_t optiga_crypt_sign(
    optiga_crypt_t *me,
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == p_digest) || (NULL == p_signature)
            || (NULL == p_signature_length)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
#endif
        ...
        return_value = optiga_cmd_calc_sign(
            me->my_cmd,
            signature_scheme,
            (optiga_calc_sign_params_t *)p_params
        );
...
// lint --e{715} suppress "The salt_length argument is kept for future use"
_STATIC_H optiga_lib_status_t optiga_crypt_verify(
    optiga_crypt_t *me,
    uint8_t cmd_param,
    const uint8_t *p_digest,
    uint8_t digest_length,
    ...
    do {
        ...
        return_value = optiga_cmd_verify_sign(
            me->my_cmd,
            (uint8_t)cmd_param,
            (optiga_verify_sign_params_t *)p_params
        );
...
_STATIC_H optiga_lib_status_t optiga_crypt_rsa_enc_dec(
    optiga_crypt_t *me,
    ...
    do {
        ...
        if (is_enc) {
            ...
            return_value = optiga_cmd_encrypt_asym(
                me->my_cmd,
            ...
            return_value = optiga_cmd_decrypt_asym(
                me->my_cmd,
                (uint8_t)encryption_scheme,
                (optiga_encrypt_asym_params_t *)p_params
            );
...
_STATIC_H optiga_lib_status_t optiga_crypt_get_random(
    optiga_crypt_t *me,
    uint8_t cmd_param,
    ...
    do {
        ...
        return_value =
            optiga_cmd_get_random(me->my_cmd, cmd_param, (optiga_get_random_params_t *)p_params);
        if (OPTIGA_LIB_SUCCESS != return_value) {
            me->instance_state = OPTIGA_LIB_INSTANCE_FREE;
        }
    } while (FALSE);
    ...
    || defined(OPTIGA_CRYPT_CLEAR_AUTO_STATE_ENABLED)
_STATIC_H optiga_lib_status_t optiga_crypt_symmetric_mode_generic(
    optiga_crypt_t *me,
    uint8_t mode,
    ...
    do {
        ...
        switch (enc_dec_type) {
            ...
            case OPTIGA_CRYPT_SYMMETRIC_ENCRYPTION: {
                ...
                return_value = optiga_cmd_encrypt_sym(
                    me->my_cmd,
            ...
            case OPTIGA_CRYPT_SYMMETRIC_DECRYPTION: {
                ...
                return_value = optiga_cmd_decrypt_sym(
                    me->my_cmd,
                    (uint8_t)p_params->mode,
...
_STATIC_H optiga_lib_status_t optiga_crypt_derive_key_generic(
    optiga_crypt_t *me,
    uint8_t type,
    uint16_t secret,
    ...
    do {
        ...
        return_value = optiga_cmd_derive_key(me->my_cmd, type, p_params);
        if (OPTIGA_LIB_SUCCESS != return_value) {
            me->instance_state = OPTIGA_LIB_INSTANCE_FREE;
        }
    } while (FALSE);
...
_STATIC_H optiga_lib_status_t optiga_crypt_hash_generic(
    optiga_crypt_t *me,
    ...
    do {
        ...
        if ((OPTIGA_CRYPT_HASH_CONTINUE == hash_sequence)
            || (OPTIGA_CRYPT_HASH_START_FINAL == hash_sequence)) {
        ...
        return_value =
            optiga_cmd_calc_hash(me->my_cmd, hash_algorithm, (optiga_calc_hash_params_t *)p_params);
        if (OPTIGA_LIB_SUCCESS != return_value) {
            me->instance_state = OPTIGA_LIB_INSTANCE_FREE;
        }
    } while (FALSE);
...
#endif
        me->my_cmd = optiga_cmd_create(optiga_instance_id, optiga_crypt_generic_event_handler, me);
        if (NULL == me->my_cmd) {
            pal_os_free(me);
            me = NULL;
        }
...
optiga_lib_status_t optiga_crypt_random(
    optiga_crypt_t *me,
...
#endif
        return_value = optiga_crypt_get_random(
            me,
            (uint8_t)rng_type,
            random_data,
            random_data_length,
...
#ifdef OPTIGA_CRYPT_HASH_ENABLED
optiga_lib_status_t optiga_crypt_hash_start(optiga_crypt_t *me, optiga_hash_context_t *hash_ctx) {
    optiga_lib_status_t return_value = OPTIGA_CRYPT_ERROR;
    OPTIGA_CRYPT_LOG_MESSAGE(__FUNCTION__);
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
...
#endif
        return_value = optiga_crypt_hash_generic(
            me,
            hash_ctx->hash_algo,
...
optiga_lib_status_t optiga_crypt_hash_update(
    optiga_crypt_t *me,
    optiga_hash_context_t *hash_ctx,
    uint8_t source_of_data_to_hash,
    const void *data_to_hash
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == hash_ctx) || (NULL == data_to_hash)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
#endif
...
optiga_lib_status_t optiga_crypt_hash_finalize(
    optiga_crypt_t *me,
    optiga_hash_context_t *hash_ctx,
    uint8_t *hash_output
) {
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == hash_ctx) || (NULL == hash_output)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
#endif
...
optiga_lib_status_t optiga_crypt_hash(
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == data_to_hash)
            || (NULL == hash_output)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
...
optiga_lib_status_t optiga_crypt_ecc_generate_keypair(
    ...
    OPTIGA_CRYPT_LOG_MESSAGE(__FUNCTION__);
    return (optiga_crypt_generate_keypair(
        me,
        (uint8_t)curve_id,
        key_usage,
        export_private_key,
...
optiga_lib_status_t optiga_crypt_ecdh(
    ...
    do {
        ...
        return_value = optiga_cmd_calc_ssec(
            me->my_cmd,
...
optiga_lib_status_t optiga_crypt_tls_prf(
    optiga_crypt_t *me,
    ...
    do {
        ...
        return_value = optiga_crypt_derive_key_generic(
            me,
            (uint8_t)type,
            secret,
            seed,
...
optiga_lib_status_t optiga_crypt_rsa_generate_keypair(
    optiga_crypt_t *me,
...
optiga_lib_status_t optiga_crypt_rsa_sign(
    optiga_crypt_t *me,
...
optiga_lib_status_t optiga_crypt_rsa_verify(
    optiga_crypt_t *me,
    optiga_rsa_signature_scheme_t signature_scheme,
    const uint8_t *digest,
    uint8_t digest_length,
...
optiga_lib_status_t optiga_crypt_rsa_encrypt_message(
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == message) || (NULL == public_key)
            || (NULL == encrypted_message) || (NULL == encrypted_message_length)) {
            break;
        }
#endif
        return_value = optiga_crypt_rsa_enc_dec(
            me,
            encryption_scheme,
            message,
            message_length,
...
optiga_lib_status_t optiga_crypt_rsa_encrypt_session(
    optiga_crypt_t *me,
    optiga_rsa_encryption_scheme_t encryption_scheme,
    const uint8_t *label,
    uint16_t label_length,
    ...
    do {
        ...
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == encrypted_message)
            || (NULL == encrypted_message_length) || (NULL == public_key)) {
            break;
        }
#endif
#ifdef OPTIGA_COMMS_SHIELDED_CONNECTION
...
#ifdef OPTIGA_CRYPT_RSA_DECRYPT_ENABLED
optiga_lib_status_t optiga_crypt_rsa_decrypt_and_export(
    optiga_crypt_t *me,
    optiga_rsa_encryption_scheme_t encryption_scheme,
...
optiga_lib_status_t optiga_crypt_rsa_decrypt_and_store(
    optiga_crypt_t *me,
    optiga_rsa_encryption_scheme_t encryption_scheme,
    const uint8_t *encrypted_message,
    uint16_t encrypted_message_length,
...
optiga_lib_status_t optiga_crypt_rsa_generate_pre_master_secret(
    ...
    do {
        ...
        if (((optional_data_length + OPTIGA_CRYPT_MINIMUM_RANDOM_DATA_LENGTH)
...
optiga_lib_status_t optiga_crypt_symmetric_encrypt(
    optiga_crypt_t *me,
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == plain_data) || (NULL == encrypted_data)
            || (NULL == encrypted_data_length)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
#endif
        return_value = optiga_crypt_symmetric_mode_generic(
            me,
...
optiga_lib_status_t optiga_crypt_symmetric_encrypt_start(
    optiga_crypt_t *me,
    optiga_symmetric_encryption_mode_t encryption_mode,
    optiga_key_id_t symmetric_key_oid,
    const uint8_t *plain_data,
...
optiga_lib_status_t optiga_crypt_symmetric_encrypt_continue(
    optiga_crypt_t *me,
    const uint8_t *plain_data,
    uint32_t plain_data_length,
    uint8_t *encrypted_data,
        ...
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == plain_data)
            || (((NULL == encrypted_data) || (NULL == encrypted_data_length))
                && (OPTIGA_SYMMETRIC_CBC_MAC
                    ...
                    )((optiga_encrypt_sym_params_t *)&(me->params.optiga_symmetric_enc_dec_params))
                            ->mode)))) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
...
optiga_lib_status_t optiga_crypt_symmetric_encrypt_final(
    optiga_crypt_t *me,
    const uint8_t *plain_data,
    uint32_t plain_data_length,
    uint8_t *encrypted_data,
...
optiga_lib_status_t optiga_crypt_symmetric_decrypt(
    optiga_crypt_t *me,
    optiga_symmetric_encryption_mode_t encryption_mode,
    optiga_key_id_t symmetric_key_oid,
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == encrypted_data) || (NULL == plain_data)
            || (NULL == plain_data_length)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
#endif
...
optiga_lib_status_t optiga_crypt_symmetric_decrypt_ecb(
    optiga_crypt_t *me,
    optiga_key_id_t symmetric_key_oid,
    const uint8_t *encrypted_data,
    uint32_t encrypted_data_length,
...
optiga_lib_status_t optiga_crypt_symmetric_decrypt_start(
    optiga_crypt_t *me,
    optiga_symmetric_encryption_mode_t encryption_mode,
    optiga_key_id_t symmetric_key_oid,
    const uint8_t *encrypted_data,
...
optiga_lib_status_t optiga_crypt_symmetric_decrypt_continue(
    optiga_crypt_t *me,
    const uint8_t *encrypted_data,
    uint32_t encrypted_data_length,
    uint8_t *plain_data,
...
optiga_lib_status_t optiga_crypt_symmetric_decrypt_final(
    optiga_crypt_t *me,
    const uint8_t *encrypted_data,
    uint32_t encrypted_data_length,
    uint8_t *plain_data,
...
optiga_lib_status_t optiga_crypt_hmac(
    optiga_crypt_t *me,
    ...
    do {
#ifdef OPTIGA_LIB_DEBUG_NULL_CHECK
        if ((NULL == me) || (NULL == me->my_cmd) || (NULL == input_data) || (NULL == mac)
            || (NULL == mac_length)) {
            return_value = OPTIGA_CRYPT_ERROR_INVALID_INPUT;
            break;
        }
...
optiga_lib_status_t optiga_crypt_hmac_start(
    optiga_crypt_t *me,
    optiga_hmac_type_t type,
    uint16_t secret,
    const uint8_t *input_data,
...
optiga_lib_status_t optiga_crypt_hmac_update(
    optiga_crypt_t *me,
    const uint8_t *input_data,
    uint32_t input_data_length
) {
...
optiga_lib_status_t optiga_crypt_hmac_finalize(
    optiga_crypt_t *me,
    const uint8_t *input_data,
    uint32_t input_data_length,
    uint8_t *mac,
...
optiga_lib_status_t optiga_crypt_hkdf(
    optiga_crypt_t *me,
...
optiga_lib_status_t optiga_crypt_symmetric_generate_key(
    ...
    do {
        ...
        return_value = optiga_cmd_gen_symkey(me->my_cmd, (uint8_t)key_type, p_params);
        if (OPTIGA_LIB_SUCCESS != return_value) {
            me->instance_state = OPTIGA_LIB_INSTANCE_FREE;
        }
...
optiga_lib_status_t optiga_crypt_generate_auth_code(
    optiga_crypt_t *me,
    optiga_rng_type_t rng_type,
    const uint8_t *optional_data,
    uint16_t optional_data_length,
...
optiga_lib_status_t optiga_crypt_hmac_verify(
    optiga_crypt_t *me,
    optiga_hmac_type_t type,
    uint16_t secret,
    const uint8_t *input_data,
...
#ifdef OPTIGA_CRYPT_CLEAR_AUTO_STATE_ENABLED
optiga_lib_status_t optiga_crypt_clear_auto_state(optiga_crypt_t *me, uint16_t secret) {
    optiga_lib_status_t return_value = OPTIGA_CRYPT_ERROR;
    OPTIGA_CRYPT_LOG_MESSAGE(__FUNCTION__);
