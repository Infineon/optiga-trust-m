...
_STATIC_H void
ifx_i2c_pl_read_register(ifx_i2c_context_t *p_ctx, uint8_t reg_addr, uint16_t reg_len);
/// Physical Layer low level interface function
_STATIC_H void ifx_i2c_pl_write_register(
...
_STATIC_H void
ifx_i2c_pl_pal_slave_addr_event_handler(void *p_input_ctx, optiga_lib_status_t event);
...
optiga_lib_status_t
ifx_i2c_pl_send_frame(ifx_i2c_context_t *p_ctx, uint8_t *p_frame, uint16_t frame_len) {
...
optiga_lib_status_t ifx_i2c_pl_write_slave_address(
    ifx_i2c_context_t *p_ctx,
...
#define DELAY_FOR_COMPLETION (0x0A)
    ...
    p_temp_upper_layer_event_handler =
        (upper_layer_callback_t *)(p_ctx->p_pal_i2c_ctx->upper_layer_event_handler);
        ...
        switch (p_ctx->pl.negotiate_state) {
            ...
            case PL_INIT_SET_FREQ_DEFAULT: {
                ...
                if (IFX_I2C_STACK_SUCCESS == event) {
                    ...
                    pal_os_event_register_callback_oneshot(
                        p_ctx->pal_os_event_ctx,
                        (register_callback)ifx_i2c_pl_negotiation_event_handler,
                        (void *)p_ctx,
                        STARTUP_TIME_MSEC
                    );
                }
    ...
    if (IFX_I2C_STACK_SUCCESS != event) {
        ...
        switch (p_ctx->pl.frame_state) {
                ...
                if ((PL_ACTION_READ_FRAME == p_ctx->pl.frame_action)
                    ...
                    if ((frame_size > 0) && (frame_size <= p_ctx->frame_size)) {
                        ...
                        if (p_ctx->dl.frame_start_time > current_time) {
                            time_stamp_diff =
                                (0xFFFFFFFF + (current_time - p_ctx->dl.frame_start_time)) + 0x01;
                        }
            ...
            case PL_STATE_RXTX: {
                ...
                p_ctx->pl.frame_state = PL_STATE_READY;
                p_ctx->pl.upper_layer_event_handler(
                    p_ctx,
...
_STATIC_H void ifx_i2c_pal_poll_callback(void *p_ctx) {
    ...
    if (PL_I2C_CMD_WRITE == p_local_ctx->pl.i2c_cmd) {
        ...
        // lint --e{534} suppress "This is the last statement of asynchronous function hence return value is not checked"
        pal_i2c_write(
            p_local_ctx->p_pal_i2c_ctx,
            p_local_ctx->pl.buffer,
...
_STATIC_H void ifx_i2c_pl_soft_reset(ifx_i2c_context_t *p_ctx) {
    ...
    switch (p_ctx->pl.request_soft_reset) {
        ...
        case PL_RESET_STARTUP: {
            ...
            pal_os_event_register_callback_oneshot(
                p_ctx->pal_os_event_ctx,
                (register_callback)ifx_i2c_pl_soft_reset,
                (void *)p_ctx,
                STARTUP_TIME_MSEC
            );
            break;
