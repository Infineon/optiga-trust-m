...
#define TRUSTM_MUTEX_DBGFN1(x, ...)    fprintf(stdout, "%d:%s:%d %s: " x "\n", getpid(),__FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#elif DEBUG_TRUSTM_MUTEX_L1 == 2

#define TRUSTM_MUTEX_DBGFN1(x, ...)    Log("%d:%s:%d %s: " x "\n", getpid(),__FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#else
#define TRUSTM_MUTEX_DBGFN1(x, ...)
#endif

#if DEBUG_TRUSTM_MUTEX_L2 == 1
#define TRUSTM_MUTEX_DBGFN2(x, ...)    fprintf(stdout, "%d:%s:%d %s: " x "\n", getpid(),__FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#elif DEBUG_TRUSTM_MUTEX_L2 == 2
#define TRUSTM_MUTEX_DBGFN2(x, ...)    Log("%d:%s:%d %s: " x "\n", getpid(),__FILE__, __LINE__, __FUNCTION__, ##__VA_ARGS__)
#else
