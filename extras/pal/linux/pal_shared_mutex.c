...
static shared_mutex_t shared_mutex_init(const char *name) {
  ...
  addr = mmap(NULL, sizeof(trustm_mutex_t), PROT_READ | PROT_WRITE, MAP_SHARED,
              mutex.shm_fd, 0);
...
pal_status_t pal_shm_mutex_acquire(shared_mutex_t *shm_mutex,
...
void pal_shm_mutex_release(shared_mutex_t *shm_mutex) {
  ...
  TRUSTM_MUTEX_DBGFN1("mutex unlock:%s:%x", shm_mutex->name,
                      (unsigned int)shm_mutex->ptr);
