...
static const uint8_t
    optiga_platform_binding_shared_secret[LENGTH_SIZE + OPTIGA_SHARED_SECRET_MAX_LENGTH] = {
...
pal_os_datastore_write(uint16_t datastore_id, const uint8_t *p_buffer, uint16_t length) {
    ...
    if (return_status == PAL_STATUS_SUCCESS) {
        ...
        if (length + offset <= FLASH_PAGE_SIZE) {
            ...
            if (flash_write(flash_dev, TEST_PARTITION_OFFSET, p_page_buffer, FLASH_PAGE_SIZE)
                != 0) {
                printk("Flash write failed!\n");
                return PAL_STATUS_FAILURE;
            }
...
pal_os_datastore_read(uint16_t datastore_id, uint8_t *p_buffer, uint16_t *p_buffer_length) {
    ...
    if (rc != 0) {
        printk(
            "Flash read failed! RC: 0x%x - Offset: 0x%x - Length %d\n",
            rc,
            offset,
    ...
    if (data_length <= max_length && data_length <= *p_buffer_length
        && data_length + LENGTH_SIZE <= FLASH_PAGE_SIZE) {
