...
static uint16_t
pal_crypt_calculate_sha256_hash(const uint8_t *message, uint16_t message_len, uint8_t *digest) {
    uint16_t status = 1;
    ...
    do {
        ...
        if (eES_SHA == sign_algo) {
            if (MBEDTLS_PK_ECKEY == key_type) {
                ...
                *sign_len =
                    (uint16_t)((ecc_ctx->grp.nbits) / 4 + ((ecc_ctx->grp.nbits) % 4 ? 2 : 0));
            } else {
...
pal_status_t pal_crypt_parse_ecc_key(
    ...
    do {
        ...
        pal_crypt_set_buffer(
            D,
            D_length,
            (uint16_t)(((ecc_ctx->grp.nbits) / 8) + ((ecc_ctx->grp.nbits) % 8 ? 1 : 0))
        );
        pal_crypt_set_buffer(
            X,
            X_length,
