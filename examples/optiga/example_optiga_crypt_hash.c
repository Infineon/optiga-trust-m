...
void example_optiga_crypt_hash(void) {
    ...
    do {
        ...
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status =
            optiga_crypt_hash_update(me, &hash_context, OPTIGA_CRYPT_HOST_DATA, &hash_data_host);
        WAIT_AND_CHECK_STATUS(return_status, optiga_lib_status);
         ...
         */
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status = optiga_crypt_hash_finalize(me, &hash_context, digest);
...
void example_optiga_crypt_hash_data(void) {
    ...
    do {
        ...
        return_status = optiga_crypt_hash(
            me,
