...
static optiga_lib_status_t optiga_generate_rsa_keypair(
    optiga_crypt_t *me,
...
static optiga_lib_status_t optiga_rsa_verify(
    optiga_crypt_t *me,
        ...
        do {
            if (0
                != mbedtls_pk_parse_key(
                    &ctx,
                    rsa_keypair[loop_count],
                    keypair_size[loop_count],
                    NULL,
                    0
                )) {
                return_status = -1;
            ...
            return_status |= mbedtls_rsa_rsassa_pkcs1_v15_verify(
                rsa_ctx,
                NULL,
                NULL,
                MBEDTLS_RSA_PUBLIC,
...
int example_mbedtls_optiga_crypt_rsa_sign() {
    ...
    do {
        ...
        for (loop_count = 0; loop_count < sizeof(hash_type) / sizeof(mbedtls_md_type_t);
             loop_count++) {
            ...
            do {
                ...
                return_status = optiga_generate_rsa_keypair(
                    me_crypt,
                ...
                return_status |= mbedtls_rsa_rsassa_pkcs1_v15_sign(
                    &rsa_ctx,
                    NULL,
                    NULL,
                    MBEDTLS_RSA_PRIVATE,
                ...
                return_status = optiga_rsa_verify(
                    me_crypt,
                    signature_scheme,
                    digest[loop_count],
...
int example_mbedtls_optiga_crypt_rsa_decrypt() {
    ...
    mbedtls_rsa_init(&rsa_ctx, MBEDTLS_RSA_PKCS_V15, 0);
    for (loop_count = 0; loop_count < sizeof(rsa_key_type) / sizeof(optiga_rsa_key_type_t);
         loop_count++) {
        public_key_length = TRUSTM_RSA_PUBLIC_KEY_MAX_SIZE;
        // mbedtls_pk_init(&ctx);
        do {
            ...
            // rsa_ctx = (mbedtls_rsa_context *)ctx.pk_ctx;
            rsa_ctx.len = rsa_key_length[loop_count];
            return_status |= mbedtls_rsa_rsaes_pkcs1_v15_decrypt(
                &rsa_ctx,
                NULL,
                NULL,
                MBEDTLS_RSA_PRIVATE,
    ...
    for (loop_count = 0; loop_count < sizeof(rsa_key_type) / sizeof(optiga_rsa_key_type_t);
        ...
        if (0
            != mbedtls_asn1_get_bitstring(
                &rsa_public_key,
                rsa_public_key + public_key_length,
                &bs
            )) {
            return_status = -1;
            break;
        ...
        if (0x01 == (*buffer_pointer & 0x7F)) {
            ...
            if (0
                != mbedtls_asn1_get_mpi(
                    &buffer_pointer,
                    buffer_pointer + public_key_modulus_length + 3,
                    &N
                )) {
                return_status = -1;
                break;
            ...
            if (0
                != mbedtls_asn1_get_mpi(
                    &buffer_pointer,
                    buffer_pointer + public_key_exponent_length + 2,
                    &E
                )) {
                return_status = -1;
                break;
            ...
            if (0
                != mbedtls_asn1_get_mpi(
                    &buffer_pointer,
                    buffer_pointer + public_key_modulus_length + 4,
                    &N
                )) {
                return_status = -1;
                break;
        ...
        return_status = mbedtls_rsa_rsaes_pkcs1_v15_encrypt(
            &rsa_ctx,
            NULL,
            NULL,
            MBEDTLS_RSA_PUBLIC,
